...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    # get the ghost position with highest belief for each living ghost
    ghostsPos = [max(beliefs, key=beliefs.get) for beliefs in livingGhostPositionDistributions]

    # get the closest ghost position
    ghostsDist = {ghostPosition: self.distancer.getDistance(pacmanPosition, ghostPosition)
                  for ghostPosition in ghostsPos}
    closestGhostPos = min(ghostsDist, key=ghostsDist.get)

    # return the action that minimizes the distance
    closestGhostDist = {action: self.distancer.getDistance(
                        Actions.getSuccessor(pacmanPosition, action), closestGhostPos) for action in legal}
    return min(closestGhostDist, key=closestGhostDist.get)
